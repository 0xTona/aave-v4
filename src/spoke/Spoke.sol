    ...
    ReentrancyGuardTransient
{
    ...
    /// @inheritdoc ISpoke
    bytes32 public constant SET_USER_POSITION_MANAGERS_TYPEHASH = EIP712Hash.SET_USER_POSITION_MANAGERS_TYPEHASH;
    ...
    uint64 internal constant HEALTH_FACTOR_LIQUIDATION_THRESHOLD = LiquidationLogic.HEALTH_FACTOR_LIQUIDATION_THRESHOLD;
    ...
    uint256 internal constant DUST_LIQUIDATION_THRESHOLD = LiquidationLogic.DUST_LIQUIDATION_THRESHOLD;
    ...
    /// @inheritdoc ISpoke
    function addDynamicReserveConfig(uint256 reserveId, DynamicReserveConfig calldata dynamicConfig)
        external
        restricted
        returns (uint32)
    {
        ...
        returns (uint256, uint256)
    {
        ...
        //      onlyPositionManager
        //      _validateSuplly()
        //  2) Pull underlying from msg.sender
        //  3) hub.add()
        //  4) userPosition.suppliedShares += suppliedShares
        ...
        returns (uint256, uint256)
    {
        ...
        //      _validateWithdraw()
        //  2) hub.remove()
        //  3) userPosition.suppliedShares -= withdrawnShares
        //  4) if using as collateral -> refresh and validate user account data + update risk premium
        ...
        returns (uint256, uint256)
    {
        ...
        //      _validateBorrow()
        //  2) hub.draw()
        //  3) userPosition.drawnShares += drawnShares
        //     set isBorrowing(reserveId) = true if not already
        //  4) refresh and validate user account data + update risk premium
        ...
        returns (uint256, uint256)
    {
        ...
        //      onlyPositionManager
        //      _validateRepay()
        //  2) Calculate drawn debt and premium debt to restore
        //  3) Pull underlying from msg.sender to hub + hub.restore()
        //  4) Update userPosition:
        //      premiumShares += premiumDelta.sharesDelta
        //      premiumOffsetRay += premiumDelta.offsetRayDelta
        //      drawnShares -= restoredShares
        //      If drawnShares == 0 -> set isBorrowing(reserveId) = false
        ...
        //2 {
        (uint256 drawnDebtRestored, uint256 premiumDebtRayRestored) =
            userPosition.calculateRestoreAmount(drawnIndex, amount);
    ...
    //}
    /// @inheritdoc ISpoke
    function setUserPositionManagersWithSig(SetUserPositionManagers calldata params, bytes calldata signature)
        external
    {
    ...
    function getUserSuppliedAssets(uint256 reserveId, address user) external view returns (uint256) {
        Reserve storage reserve = _reserves.get(reserveId);
        return reserve.hub.previewRemoveByShares(reserve.assetId, _userPositions[user][reserveId].suppliedShares);
    }
        ...
        returns (uint256)
    {
        ...
        return LiquidationLogic.calculateLiquidationBonus({
            ...
            maxLiquidationBonus: _dynamicConfig[reserveId][_userPositions[user][reserveId].dynamicConfigKey].maxLiquidationBonus
        });
    }
    ...
    function _refreshAndValidateUserAccountData(address user) internal returns (UserAccountData memory) {
        UserAccountData memory accountData = _processUserAccountData(user, true);
        emit RefreshAllUserDynamicConfig(user);

        require(accountData.healthFactor >= HEALTH_FACTOR_LIQUIDATION_THRESHOLD, HealthFactorBelowThreshold());
        return accountData;
    }
    ...
    function _processUserAccountData(address user, bool refreshConfig)
        internal
        returns (UserAccountData memory accountData)
    {
        //@note
        //Intention
        //  1) Process all active user positions
        //      1.1) If collateral:
        //          If refreshConfig -> update userPosition.dynamicConfigKey
        //          userCollateralValue = toAsset(suppliedShares) * assetPrice * 10**(18 - assetDecimals)
        //          avgCollateralFactor = userCollateralValue * collateralFactor
        //            -> divide by totalCollateralValue later
        //      1.2) If borrowing:
        //          debtRay = drawnShares * drawnIndex + premiumDebtRay
        //          totalDebtValueRay += debtRay * assetPrice * 10**(18 - assetDecimals)
        //  2) Calculate health factor
        //      If totalDebtValueRay > 0 -> healthFactor = totalWeightedCollateralValueRay * RAY / totalDebtValueRay
        //                                    where: totalWeightedCollateralValueRay = avgCollateralFactor (not yet divided)
        //      Else                     -> healthFactor = type(uint256).max
        //  3) Normalize avgCollateralFactor
        //      If totalCollateralValue > 0 -> avgCollateralFactor /= totalCollateralValue
        //  4) Calculate risk premium
        //      4.1) Sort collateralInfo by collateral risk (ASC) and value (DESC)
        //      4.2) Iterate sorted collaterals to cover total debt, accumulating riskPremium += userCollateralValue * collateralRisk
        //      4.3) Normalize riskPremium: riskPremium /= totalUserCollateralValueUsedToCoverDebt

        PositionStatus storage positionStatus = _positionStatus[user];

        uint256 reserveId = _reserveCount;
        KeyValueList.List memory collateralInfo = KeyValueList.init(positionStatus.collateralCount(reserveId));
        bool borrowing;
        bool collateral;
        ...
        while (true) {
            ...
            if (collateral) {
                uint256 collateralFactor =
                    _dynamicConfig[reserveId][refreshConfig
                        ? (userPosition.dynamicConfigKey = reserve.dynamicConfigKey)
                        : userPosition.dynamicConfigKey].collateralFactor;
                if (collateralFactor > 0) {
                    ...
                    if (suppliedShares > 0) {
                        ...
                        collateralInfo.add(
                            accountData.activeCollateralCount, reserve.collateralRisk, userCollateralValue
                        );
            ...
            if (borrowing) {
                UserPositionUtils.DebtComponents memory debtComponents =
                    userPosition.getDebtComponents(reserve.hub, reserve.assetId);
                uint256 debtRay = debtComponents.drawnShares * debtComponents.drawnIndex + debtComponents.premiumDebtRay;

                accountData.totalDebtValueRay += debtRay.toValue({decimals: assetDecimals, price: assetPrice});
    ...
    function _notifyRiskPremiumUpdate(address user, uint256 newRiskPremium) internal {
        //@note
        //Intention
        //  1) If newRiskPremium = 0 && positionStatus.riskPremium = 0 -> return
        //  2) Else update each user's debt premium with newRiskPremium
    ...
    function _isAssetIdListed(address hub, uint256 assetId, uint256 reserveId) internal view returns (bool) {
    ...
    function _validateDynamicReserveConfig(DynamicReserveConfig calldata config) internal pure {
        require(
            config.collateralFactor < PercentageMath.PERCENTAGE_FACTOR
                ...
                && config.maxLiquidationBonus.percentMulUp(config.collateralFactor) < PercentageMath.PERCENTAGE_FACTOR,
    ...
    function _castToView(function(address, bool) internal returns (UserAccountData memory) fnIn)
        internal
        pure
