...
library UserPositionUtils {
    ...
    function applyPremiumDelta(ISpoke.UserPosition storage userPosition, IHubBase.PremiumDelta memory premiumDelta)
        internal
    {
        //@note
        //Intention
        //  userPosition.premiumShares += premiumDelta.sharesDelta
        //  userPosition.premiumOffsetRay += premiumDelta.offsetRayDelta

        userPosition.premiumShares = userPosition.premiumShares.add(premiumDelta.sharesDelta).toUint120();
        userPosition.premiumOffsetRay = (userPosition.premiumOffsetRay + premiumDelta.offsetRayDelta).toInt200();
    }
    ...
    function calculatePremiumDelta(
        ...
        //@note
        //Intention
        //  sharesDelta = newPremiumShares - oldPremiumShares
        //      Where:
        //        newPremiumShares = (userPosition.drawnShares  - drawnSharesTaken) * riskPremium / 1e4
        ...
        //           premiumDebtRay = oldPremiumShares * drawnIndex - oldPremiumOffsetRay
        ...
        uint256 premiumDebtRay = Premium.calculatePremiumRay({
            premiumShares: oldPremiumShares, premiumOffsetRay: oldPremiumOffsetRay, drawnIndex: drawnIndex
        });
    ...
    function calculateRestoreAmount(ISpoke.UserPosition storage userPosition, uint256 drawnIndex, uint256 amount)
        internal
        view
        returns (uint256, uint256)
    {
        //@note
        //Intention
        //  Prioritize restoring drawn debt first, then premium debt.
        //Notice
        //  abundant amount will be ignored
    ...
    function getDebt(ISpoke.UserPosition storage userPosition, IHubBase hub, uint256 assetId)
        internal
        view
        returns (uint256, uint256)
    {
    ...
    /// @return The user's drawn debt, expressed in asset units.
    /// @return The user's premium debt, expressed in asset units and scaled by RAY.
    function getDebt(ISpoke.UserPosition storage userPosition, uint256 drawnIndex)
        internal
        view
        returns (uint256, uint256)
    {
    ...
    function getDebtComponents(ISpoke.UserPosition storage userPosition, IHubBase hub, uint256 assetId)
        internal
        view
        returns (DebtComponents memory)
    {
    ...
    function _calculatePremiumRay(ISpoke.UserPosition storage userPosition, uint256 drawnIndex)
        internal
        view
        returns (uint256)
    {
