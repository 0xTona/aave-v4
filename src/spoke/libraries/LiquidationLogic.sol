...
library LiquidationLogic {
    ...
    function liquidateUser(
        ...
        mapping(
            address user
                => mapping(
                uint256 reserveId => ISpoke.UserPosition
            )
        ) storage userPositions,
        mapping(address user => ISpoke.PositionStatus) storage positionStatus,
        mapping(
            uint256 reserveId => mapping(uint32 dynamicConfigKey => ISpoke.DynamicReserveConfig)
        ) storage dynamicConfig,
        ...
        ISpoke.UserPosition storage collateralUserPosition = userPositions[params.user][params.collateralReserveId];
        ISpoke.DynamicReserveConfig storage collateralDynConfig =
            dynamicConfig[params.collateralReserveId][collateralUserPosition.dynamicConfigKey];
    ...
    function notifyReportDeficit(
        ...
        while ((reserveId = userPositionStatus.nextBorrowing(reserveId)) != PositionStatusMap.NOT_FOUND) {
            ...
            UserPositionUtils.DebtComponents memory debtComponents = userPosition.getDebtComponents(hub, assetId);
            ...
            hub.reportDeficit(assetId, debtComponents.drawnShares.rayMulUp(debtComponents.drawnIndex), premiumDelta);
    ...
    function calculateLiquidationBonus(
        ...
        //@note
        //Intention
        //  1) If healthFactor <= healthFactorForMaxBonus -> return maxLiquidationBonus
        //  2) Else                                       -> linearly interpolate bonus based on healthFactor
        //          minBonus = (maxLiquidationBonus - 100%) * liquidationBonusFactor / 1e4 + 100%
        //          return minBonus + (maxLiquidationBonus - minBonus) * (1.0 - HF) / (1.0 - HF_for_max)
    ...
    function _executeLiquidation(
        ...
        UserPositionUtils.DebtComponents memory debtComponents =
            debtUserPosition.getDebtComponents(params.debtHub, params.debtAssetId);
    ...
    function _liquidateCollateral(
        ...
        //Intention
        //  1) Liquidator:
        //      If receiveShares -> liquidatorPosition.suppliedShares += sharesToLiquidator
        //      Else             -> hub.remomve(amountToLiquidator)
        //  2) Protocol fee: hub.payFeeShares()

        uint120 newUserSuppliedShares = userPosition.suppliedShares - params.sharesToLiquidate.toUint120();
        userPosition.suppliedShares = newUserSuppliedShares;

        uint256 amountRemoved = params.hub.previewRemoveByShares(params.assetId, params.sharesToLiquidate);
        ...
        if (params.sharesToLiquidator > 0) {
            if (params.receiveShares) {
                ...
                if (params.sharesToLiquidator < params.sharesToLiquidate) {
                    amountToLiquidator = params.hub.previewRemoveByShares(params.assetId, params.sharesToLiquidator);
                }
        ...
        return LiquidateCollateralResult({
            amountRemoved: amountRemoved, isCollateralPositionEmpty: newUserSuppliedShares == 0
        });
    }
    ...
    function _liquidateDebt(
        ...
        IERC20(params.underlying).safeTransferFrom(params.liquidator, address(params.hub), amountToRestore);
        ...
        return LiquidateDebtResult({
            amountRestored: amountToRestore, premiumDelta: premiumDelta, isDebtPositionEmpty: isDebtPositionEmpty
        });
    }
    ...
    function _validateLiquidationCall(ValidateLiquidationCallParams memory params) internal pure {
        ...
        require(!params.collateralReserveFlags.paused() && !params.debtReserveFlags.paused(), ISpoke.ReservePaused());
        ...
        require(params.healthFactor < HEALTH_FACTOR_LIQUIDATION_THRESHOLD, ISpoke.HealthFactorNotBelowThreshold());
        require(params.collateralFactor > 0 && params.isUsingAsCollateral, ISpoke.ReserveNotEnabledAsCollateral());
        if (params.receiveShares) {
            require(
                !params.collateralReserveFlags.frozen() && params.collateralReserveFlags.receiveSharesEnabled(),
    ...
    /// @dev Invoked by `liquidateUser` method.
    function _calculateLiquidationAmounts(CalculateLiquidationAmountsParams memory params)
        internal
        view
        returns (LiquidationAmounts memory)
    {
        //@note
        //Intention
        //  1) Calculate liquidation bonus
        //  2) Calculate debt amounts needed to reach target health factor
        //  3) Calculate corresponding collateral shares
        //  4) Check if partial collateral liquidation leaves collateral dust
        //  5) If collateral needed > supplied || leaves collateral dust -> recalculate collateral and debt to liquidate to ensure full liquidation of collateral
        //          Guard: if debtToCover < newDebtToLiquidate -> revert
        //  7) Calculate collateral shares for liquidator after deducting protocol fee
        ...
        if (collateralSharesToLiquidate < params.suppliedShares) {
            uint256 collateralRemaining = params.collateralReserveHub
                .previewRemoveByShares(
                    params.collateralReserveAssetId, params.suppliedShares.uncheckedSub(collateralSharesToLiquidate)
                );
            leavesCollateralDust = collateralRemaining.toValue({
                decimals: params.collateralAssetDecimals, price: params.collateralAssetPrice
        ...
        if (
            ...
            uint256 debtRayToLiquidate = Math.mulDiv(
                params.collateralReserveHub
                    .previewAddByShares(params.collateralReserveAssetId, collateralSharesToLiquidate),
                params.collateralAssetPrice * debtAssetUnit * PercentageMath.PERCENTAGE_FACTOR * WadRayMath.RAY,
            ...
            if (debtRayToLiquidate <= params.premiumDebtRay) {
                ...
                drawnSharesToLiquidate = (debtRayToLiquidate - premiumDebtRayToLiquidate).divUp(params.drawnIndex);
    ...
    function _calculateCollateralToLiquidate(CalculateCollateralToLiquidateParams memory params)
        internal
        view
        returns (uint256)
    {
        ...
        //  collateralToLiquidate = (debtRayToLiquidate * debtPrice * bonus) / (collatPrice * 1e4 * RAY) * (collatUnit / debtUnit)
        //  collateralSharesToLiquidate = previewAddByAssets(collateralToLiquidate)
        ...
        uint256 collateralToLiquidate = Math.mulDiv(
            ...
            params.debtAssetUnit * params.collateralAssetPrice * PercentageMath.PERCENTAGE_FACTOR * WadRayMath.RAY,
            Math.Rounding.Floor
        );

        uint256 collateralSharesToLiquidate =
            params.collateralReserveHub.previewAddByAssets(params.collateralReserveAssetId, collateralToLiquidate);
    ...
    function _calculateDebtToLiquidate(CalculateDebtToLiquidateParams memory params)
        internal
        pure
        returns (uint256, uint256)
    {
        //@note
        //Intention
        //  1) Calculate ideal debt needed to reach target health factor (debtRayToTarget)
        //  2) Calculate premium debt to liquidate (liquidated first)
        //      premiumDebtRayToLiquidate = min(debtRayToTarget.roundRayUp(), params.premiumDebtRay, params.debtToCover.toRay())
        //  3) Calculate drawn shares to liquidate
        //      If debtRayToTarget > premiumDebtRayToLiquidate -> calculate drawn shares needed for remaining debt:
        //                                                          drawnSharesToLiquidate = min(drawnSharesToTarget, drawnSharesToCover)
        //                                                                  drawnSharesToTarget = ceil(debtRayToTarget - premiumDebtRayToLiquidate) / drawnIndex
        //                                                                  drawnSharesToCover  = (debtToCover - premiumDebtRayToLiquidate.fromRayUp()) * RAY / drawnIndex
        //  4) If leavesDebtDust -> bypass target health factor and fully liquidate all debt (premium + drawn shares)
        ...
        //3 {
        uint256 drawnSharesToLiquidate;
        if (premiumDebtRayToLiquidate == params.premiumDebtRay && premiumDebtRayToLiquidate < debtRayToTarget) {
            uint256 drawnSharesToTarget = (debtRayToTarget - premiumDebtRayToLiquidate).divUp(params.drawnIndex);
        ...
        uint256 debtRayRemaining = (params.drawnShares - drawnSharesToLiquidate) * params.drawnIndex
            + params.premiumDebtRay - premiumDebtRayToLiquidate;
    ...
    function _calculateDebtToTargetHealthFactor(CalculateDebtToTargetHealthFactorParams memory params)
        internal
        pure
        returns (uint256)
    {
        //@note
        //Intention
        //  Calculate deltaDebt that after liquidation: HF_target = (totalCollateralValue - deltaDebt * bonus * collateralFactor) / (totalDebtValue - deltaDebt)
        //                                         <=>  HF_target = (totalDebtValue * HF_current - deltaDebt * bonus * collateralFactor) / (totalDebtValue - deltaDebt)
        //                                         <=>  HF_target * (totalDebtValue - deltaDebt) = totalDebtValue * HF_current - deltaDebt * bonus * collateralFactor
        //                                         <=>  totalDebtValue * (HF_target - HF_current) = deltaDebt * (HF_target - bonus * collateralFactor)
        //                                         <=>  deltaDebt = totalDebtValue * (HF_target - HF_current) / (HF_target - bonus * collateralFactor)
