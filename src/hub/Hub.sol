...
abstract contract Hub is IHub, HubStorage, AccessManagedUpgradeable {
    ...
    function addAsset(
        ...
        require(underlying != address(0) && feeReceiver != address(0) && irStrategy != address(0), InvalidAddress());
        require(
        ...
        uint256 drawnRate = IBasicInterestRateStrategy(irStrategy)
            .calculateInterestRate({assetId: assetId, liquidity: 0, drawn: 0, deficit: 0, swept: 0});
        ...
        emit UpdateAssetConfig(
            assetId,
            AssetConfig({
                feeReceiver: feeReceiver, liquidityFee: 0, irStrategy: irStrategy, reinvestmentController: address(0)
            })
        );
    ...
    /// @inheritdoc IHub
    function updateAssetConfig(uint256 assetId, AssetConfig calldata config, bytes calldata irData)
        external
        restricted
    {
        ...
        require(config.reinvestmentController != address(0) || asset.swept == 0, InvalidReinvestmentController());
    ...
    /// @inheritdoc IHub
    function addSpoke(uint256 assetId, address spoke, SpokeConfig calldata config) external restricted {
        require(assetId < _assetCount, AssetNotListed());
    ...
    /// @inheritdoc IHub
    function updateSpokeConfig(uint256 assetId, address spoke, SpokeConfig calldata config) external restricted {
        require(assetId < _assetCount, AssetNotListed());
    ...
    function add(uint256 assetId, uint256 amount) external returns (uint256) {
        ...
        //      shares > 0
        //  3) Update:
        //      asset.addedShares, spoke.addedShares += shares
        //      liquidity += amount
        //      updateDrawnRate()
    ...
    /// @inheritdoc IHub
    function eliminateDeficit(uint256 assetId, uint256 amount, address spoke)
        external
        restricted
        returns (uint256, uint256)
    {
    ...
    function _addFeeReceiver(uint256 assetId, address feeReceiver) internal {
        ...
        _updateSpokeConfig(
            ...
            feeReceiver,
            SpokeConfig({
                addCap: MAX_ALLOWED_SPOKE_CAP, drawCap: 0, riskPremiumThreshold: 0, active: true, halted: false
            })
        );
    }
    ...
    function _transferShares(SpokeData storage sender, SpokeData storage receiver, uint256 shares) internal {
    ...
    function _applyPremiumDelta(Asset storage asset, SpokeData storage spoke, PremiumDelta calldata premiumDelta)
        internal
    {
        uint256 drawnIndex = asset.drawnIndex;

        // asset premium change
        (asset.premiumShares, asset.premiumOffsetRay) =
            _validateApplyPremiumDelta(drawnIndex, asset.premiumShares, asset.premiumOffsetRay, premiumDelta);

        // spoke premium change
        (spoke.premiumShares, spoke.premiumOffsetRay) =
            _validateApplyPremiumDelta(drawnIndex, spoke.premiumShares, spoke.premiumOffsetRay, premiumDelta);
    ...
    function _getSpokePremium(Asset storage asset, SpokeData storage spoke) internal view returns (uint256) {
    ...
    function _validateAdd(Asset storage asset, SpokeData storage spoke, uint256 amount) internal view {
    ...
    function _validateDraw(Asset storage asset, SpokeData storage spoke, uint256 amount, address to) internal view {
        require(to != address(this), InvalidAddress());
        require(amount > 0, InvalidAmount());
        require(spoke.active, SpokeNotActive());
        require(!spoke.halted, SpokeHalted());
    ...
    function _validateEliminateDeficit(SpokeData storage callerSpoke, uint256 deficitAmountRay) internal view {
    ...
    function _validateApplyPremiumDelta(
        ...
        uint256 premiumRayBefore = Premium.calculatePremiumRay({
            premiumShares: premiumShares, premiumOffsetRay: premiumOffsetRay, drawnIndex: drawnIndex
        });
        ...
        uint256 premiumRayAfter = Premium.calculatePremiumRay({
            premiumShares: newPremiumShares, premiumOffsetRay: newPremiumOffsetRay, drawnIndex: drawnIndex
        });
