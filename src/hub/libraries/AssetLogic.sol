...
library AssetLogic {
    ...
    function toDrawnAssetsUp(IHub.Asset storage asset, uint256 shares) internal view returns (uint256) {
    ...
    function toDrawnAssetsDown(IHub.Asset storage asset, uint256 shares) internal view returns (uint256) {
    ...
    function toDrawnSharesUp(IHub.Asset storage asset, uint256 assets) internal view returns (uint256) {
    ...
    function toDrawnSharesDown(IHub.Asset storage asset, uint256 assets) internal view returns (uint256) {
    ...
    function premium(IHub.Asset storage asset, uint256 drawnIndex) internal view returns (uint256) {
        return Premium.calculatePremiumRay({
            premiumShares: asset.premiumShares, drawnIndex: drawnIndex, premiumOffsetRay: asset.premiumOffsetRay
        }).fromRayUp();
    }
    ...
    function toAddedAssetsUp(IHub.Asset storage asset, uint256 shares) internal view returns (uint256) {
    ...
    function toAddedAssetsDown(IHub.Asset storage asset, uint256 shares) internal view returns (uint256) {
    ...
    function toAddedSharesUp(IHub.Asset storage asset, uint256 assets) internal view returns (uint256) {
    ...
    function toAddedSharesDown(IHub.Asset storage asset, uint256 assets) internal view returns (uint256) {
    ...
    function getDrawnIndex(IHub.Asset storage asset) internal view returns (uint256) {
        //@note
        //Intention
        //  1) If lastUpdateTimestamp == block.timestamp || no premium & drawn shares => return previousIndex
        //  2) Else                                                                   -> return previousIndex * (drawnRate * deltaTime / SECONDS_PER_YEAR + RAY) / RAY
    ...
    function getDrawnRate(IHub.Asset storage asset, uint256 assetId, uint256 drawnIndex)
        internal
        view
        returns (uint256)
    {
        return IBasicInterestRateStrategy(asset.irStrategy)
    ...
    function getUnrealizedFees(IHub.Asset storage asset, uint256 drawnIndex) internal view returns (uint256) {
        //@note
        //Intention
        //  1) Guard:
        //      previousIndex == drawnIndex => return 0
        //      liquidityFee == 0           => return 0
        //  2) Calculate:
        //      aggregatedOwedRayAfter = drawnShares * drawnIndex + (premiumShares * drawnIndex - premiumOffsetRay) + deficitRay
        //      aggregatedOwedRayBefore = drawnShares * previousIndex + (premiumShares * previousIndex - premiumOffsetRay) + deficitRay
        //      -> return (aggregatedOwedRayAfter - aggregatedOwedRayBefore).percentMulDown(liquidityFee)
        //              = (drawnShares + premiumShares * (drawnIndex - previousIndex) - premiumOffsetRay + deficitRay) * liquidityFee / 1e4
        ...
        return (aggregatedOwedRayAfter.fromRayUp() - aggregatedOwedRayBefore.fromRayUp()).percentMulDown(liquidityFee);
    }
    ...
    function _calculateAggregatedOwedRay(
        ...
        //@note
        //Intention
        //  drawShares * drawnIndex + (premiumShares * drawnIndex - premiumOffsetRay) + deficitRay
